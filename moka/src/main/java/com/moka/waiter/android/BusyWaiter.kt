...
class BusyWaiter private constructor(private val completedOnExecutionThread: ExecutionThread) {
    ...
    @GuardedBy("lock")
    private val operationsInProgress = com.moka.waiter.android.internal.SetMultiMap<Category, Any>()
    @GuardedBy("lock")
    ...
    private val defaultCategory = Category.GENERAL
    ...
    fun busyWith(operation: Any, category: Category) {
        ...
        try {
            ...
            if (wasAdded) {
                Timber.i("busyWith -> [$operation] was added to active operations")
            }
        } finally {
            lock.unlock()
        }
    ...
    private fun completed(operation: Any?, iterator: MutableIterator<Any>?) {
        ...
        try {
            ...
            if (wasRemoved) {
                Timber.i("completed -> [${operation}] was removed from active operations")
            }
            if (wasRemoved && isNotBusy) {
                for (noLongerBusyCallback in noLongerBusyCallbacks) {
                    Timber.i("All operations are now finished, we are now idle")
